...
// Defining the shape of filter object
interface Filter {
  [key: string]: string[];
}
...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    if (headers) {
      ...
      const worker = new Worker(
        new URL("../web-workers/file-upload.worker.ts", import.meta.url)
      );
  ...
  useEffect(() => {
    if (headers && queryParams) {
      ...
      const worker = new Worker(
        new URL("../web-workers/filter.worker.ts", import.meta.url)
      );
      const messageData = {
        allTableData,
        allOptions,
    ...
    if (Object.keys(dropdownOptions).length && !hasRunOnce.current) {
      ...
      for (let [header, index] of Object.entries(bigOptions.current)) {
        dropdowns[index].addEventListener("scroll", function () {
          if (
            dropdowns[index].scrollHeight - dropdowns[index].scrollTop ===
            dropdowns[index].clientHeight
          ) {
  ...
  // reset handler for error-boundary component
  const resetHandler = () => {
    filter.current = {};
    navigate("/");
    setFileData([]);
